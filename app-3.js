...
const LIB = [
  // Barbell
  ["Barbell Bench Press","Barbell","Squeeze shoulder blades together, lower bar to mid-chest, drive feet into floor."],
  ["Back Squat","Barbell","Brace your core, sit back and down, keep knees tracking over toes."],
  ["Deadlift","Barbell","Bar over mid-foot, flat back, push the floor away. Don't yank it."],
  ["Overhead Press","Barbell","Squeeze glutes, bar travels straight up past your nose, finish overhead."],
  ["Barbell Row","Barbell","Hinge ~45°, pull to lower ribs, control the lowering."],
  ["Romanian Deadlift","Barbell","Soft knees, push hips back, feel the hamstring stretch, flat back."],
  ["Front Squat","Barbell","Elbows high, upright torso, drive up through mid-foot."],
  // Dumbbell
  ["Dumbbell Bench Press","Dumbbell","Lower deep for stretch, press up and slightly together at the top."],
  ["Dumbbell Shoulder Press","Dumbbell","Press up without flaring elbows too wide, control the descent."],
  ["Dumbbell Curl","Dumbbell","Keep elbows pinned, no swinging, squeeze at the top."],
  ["Dumbbell Row","Dumbbell","Flat back, pull elbow past your ribs, slow on the way down."],
  ["Lateral Raise","Dumbbell","Lead with elbows, raise to shoulder height, light weight wins here."],
  ["Dumbbell Lunge","Dumbbell","Step into a tall lunge, back knee toward floor, drive through front heel."],
  ["Incline Dumbbell Press","Dumbbell","30-45° bench, lower under control, press without locking hard."],
  ["Hammer Curl","Dumbbell","Neutral grip, elbows fixed, controlled tempo both directions."],
  // Machine
  ["Leg Press","Machine","Feet shoulder-width, don't lock knees at top, control the negative."],
  ["Lat Pulldown","Machine","Pull bar to upper chest, drive elbows down, don't lean back too far."],
  ["Seated Cable Row","Machine","Chest up, pull to your stomach, squeeze, slow return."],
  ["Leg Curl","Machine","Full range, squeeze hamstrings at the top, no jerking."],
  ["Leg Extension","Machine","Pause at the top, control down, don't slam the stack."],
  ["Chest Press Machine","Machine","Set seat so handles align mid-chest, press without shrugging."],
  ["Pec Deck","Machine","Slight elbow bend, squeeze chest, smooth arc both ways."],
  ["Cable Tricep Pushdown","Machine","Elbows glued to sides, full extension, control back up."],
  // Bodyweight
  ["Pull-up","Bodyweight","Dead hang start, pull chest to bar, control the lowering."],
  ["Push-up","Bodyweight","Body in a straight line, elbows ~45°, full range each rep."],
  ["Dip","Bodyweight","Lean slightly forward, lower until shoulders ~parallel, press up."],
  ["Plank","Bodyweight","Straight line head to heels, brace, breathe. Log seconds in reps."],
  ["Bodyweight Squat","Bodyweight","Sit back and down, chest up, full depth, control the rise."],
  ["Chin-up","Bodyweight","Underhand grip, pull till chin clears bar, slow negative."],
];
const CATS = ["All","Barbell","Dumbbell","Machine","Bodyweight"];
const CAT_COLOR = {Barbell:"#ff3da6",Dumbbell:"#2dd4ff",Machine:"#a855f7",Bodyweight:"#5b8cff",Custom:"#ff7ad9"};
...
function renderStats(){
  ...
  <div class="section-h"><h2>Where your volume goes</h2></div>
...
/* ============ PICKER ============ */
let pickerCat="All", pickerQ="", pickerList=[];
function openPicker(){
  if(!state.active)startWorkout();
  pickerCat="All";pickerQ="";
...
function renderPicker(){
  ...
  pickerList=LIB.filter(l=>(pickerCat==="All"||l[1]===pickerCat)&&l[0].toLowerCase().includes(q));
...
function addByIndex(i){const l=pickerList[i];if(l)addExercise(l[0],l[1],l[2]);}
function addCustom(){const n=pickerQ.trim();if(n)addExercise(n,"Custom","");}
function addExercise(name,cat,tip){
  state.active.exercises.push({name,cat,tip,sets:[{w:0,r:cat==="Bodyweight"?8:0,done:false}]});
...
function loadSampleData(){
  ...
  const e=(name,cat,sets)=>{const tip=(findEx(name)||[])[2]||"";return{name,cat,tip,sets:sets.map(([w,r])=>({w,r,done:true}))};};
  state.workouts=[
