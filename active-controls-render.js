...
(function(){
  ...
  renderExercises=function(a){
    ...
    a.exercises.forEach((e,ei)=>{
      ...
      const top=lastTopSet(e.name,a.editingIndex),isbw=e.cat==="Bodyweight",pendingIndex=Math.max(0,e.sets.findIndex(s=>!s.done));
      ...
      html+=`<div class="fac-ex-tools"><button onclick="facOpenQuickNote(${ei})"><span>✎</span>${e.note?'View note':'Quick note'}</button><button onclick="facOpenReplaceExercise(${ei})"><span>↻</span>Replace</button>${FAC.isBarbell(e)?`<button onclick="facOpenPlateCalculator(${ei},${pendingIndex})"><span>◉</span>Plates</button>`:""}</div>${e.note?`<button class="fac-note-preview" onclick="facOpenQuickNote(${ei})">${esc(e.note)}</button>`:""}<div class="sets"><div class="colhead fac-colhead"><div>#</div><div>${isbw?'Added '+state.unit:'Weight'}</div><div>${isbw?'Reps / sec':'Reps'}</div><div>WU</div><div></div></div>`;
      e.sets.forEach((s,si)=>{
        if(s.done){
